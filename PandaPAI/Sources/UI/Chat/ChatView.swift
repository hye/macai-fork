...
struct ChatView: View {
    ...
    var backgroundColor = Color.primary.opacity(0.05)
    ...
    var body: some View {
        ...
        mainContent
            ...
            .navigationTitle(
                chat.name != "" ? chat.name : chat.persona?.name ?? "PandaPAI"
            )
    ...
    private var mainContent: some View {
        VStack(spacing: 0) {
            ...
            ChatInputView(
                ...
                fileUploadsAllowed: chat.apiService?.imageUploadsAllowed ?? false,
                onSendMessage: {
                    logicHandler.sendMessage(messageText: newMessage, attachedFiles: attachedFiles)
                    newMessage = ""
                    attachedFiles = []
                ...
                onAddFile: {
                    #if os(iOS)
                    // On iOS, we'll handle document picker in the view
                    #else
                    logicHandler.selectAndAddFiles { newAttachments in
                        withAnimation {
            ...
            .id(chat.apiService?.imageUploadsAllowed) // Force re-render when service's imageUploadsAllowed changes
        }
    }
