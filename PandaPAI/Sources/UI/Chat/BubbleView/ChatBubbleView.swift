...
struct ChatBubbleView: View, Equatable {
    ...
    private let incomingLabelColor = Color.primary
            ...
            HStack {
                ...
                .background(
                    content.systemMessage
                        ? (color != nil ? (Color(hex: color!) ?? Color.gray) : Color.gray).opacity(0.6)
                        : colorScheme == .dark
    ...
    private var toolbarContent: some View {
        HStack(spacing: 12) {
            ...
            if let messageEntity = message {
                ShareLink(item: ShareUtils.formatMessageForSharing(messageEntity)) {
                    ToolbarButton(
