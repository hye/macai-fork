...
class ChatLogicHandler: ObservableObject {
    ...
    func sendMessage(messageText: String, attachedFiles: [FileAttachment]) {
    ...
    func selectAndAddFiles(completion: @escaping ([FileAttachment]) -> Void) {
        ...
        panel.allowedContentTypes = [.jpeg, .png, .heic, .heif, .pdf, .text, .json, UTType(filenameExtension: "webp")].compactMap { $0 }
        panel.title = "Select Files"
        panel.message = "Choose files to upload"

        panel.begin { response in
            if response == .OK {
                var newAttachments: [FileAttachment] = []
        ...
        #else
        // On iOS, use document picker
        // This will be handled by the SwiftUI view using DocumentPickerView
        // For now, we'll create a placeholder that can be called from the view
        #endif
    }
    ...
    func handleRetryMessage(newMessage: inout String) {
        ...
        if currentError != nil {
            sendMessage(messageText: newMessage, attachedFiles: [])
        } else {
