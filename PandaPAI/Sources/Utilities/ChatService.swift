...
class ChatService {
    ...
    var lastMessageError: Bool = false
    ...
    func sendMessage(_ message: String?, completion: @escaping (Result<MessageEntity, ChatError>) -> Void) {
        ...
        let task = session.dataTask(with: request) { data, response, error in
            DispatchQueue.main.async {
                ...
                do {
                    let messageEntity = try self.processResponse(data: data, response: response, error: error)
    ...
    func processResponse(data: Data?, response: URLResponse?, error: Error?) throws -> MessageEntity {
        ...
        if let responseString = String(data: data, encoding: .utf8) {
                ...
                let messageContent = content["content"] as? String
            {
                ...
                throw ChatError.jsonParsingFailure
            }
        } else {
            throw ChatError.invalidData
        }
