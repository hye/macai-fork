...
class OpenRouterHandler: ChatGPTHandler {    
        ...
        if let responseString = String(data: data, encoding: .utf8) {
            ...
            do {
                    ...
                    let messageRole = message["role"] as? String
                {
                    ...
                    // Handle content as string (OpenAI format) or array (multimodal format)
                    if let messageContent = message["content"] as? String {
                        finalContent = messageContent
                    } else if let contentArray = message["content"] as? [[String: Any]] {
                        finalContent = contentArray.compactMap { item -> String? in
        ...
        do {
                ...
                let delta = firstChoice["delta"] as? [String: Any]
            {
                ...
                if let contentPart = delta["content"] as? String {
                    content = contentPart
                } else if let contentArray = delta["content"] as? [[String: Any]] {
                    content = contentArray.compactMap { item -> String? in
                        if let type = item["type"] as? String, type == "text",
                            let text = item["text"] as? String
                        {
                            return text
    ...
    override func sendMessageStream(_ requestMessages: [[String: Any]], temperature: Float) async throws
    ...
    override internal func prepareRequest(requestMessages: [[String: Any]], model: String, temperature: Float, stream: Bool) -> URLRequest {
        let filteredMessages = requestMessages.map { message -> [String: Any] in
