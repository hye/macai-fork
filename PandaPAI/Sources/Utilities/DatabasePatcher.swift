...
class DatabasePatcher {
    ...
    static func migrateExistingConfiguration(context: NSManagedObjectContext) {
        ...
        let defaults = UserDefaults.standard
        let migrationCompleted = defaults.bool(forKey: "APIServiceMigrationCompleted")
        if migrationCompleted {
            return
        }

        // Check if there are any legacy settings to migrate
        let hasLegacyApiUrl = defaults.object(forKey: "apiUrl") != nil
        let hasLegacyGptToken = defaults.object(forKey: "gptToken") != nil
        let hasLegacyGptModel = defaults.object(forKey: "gptModel") != nil

        // Only create API service if there are legacy settings OR if this is a fresh install with no existing API services
        let existingApiServicesCount = (try? context.count(for: NSFetchRequest<APIServiceEntity>(entityName: "APIServiceEntity"))) ?? 0

        if !hasLegacyApiUrl && !hasLegacyGptToken && !hasLegacyGptModel && existingApiServicesCount == 0 {
