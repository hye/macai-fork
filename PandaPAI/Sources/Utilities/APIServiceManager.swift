...
class APIServiceManager {
    ...
    /// 檢查 APIServiceEntity 是否準備好使用（有 URL）
    func isAPIServiceReady(_ apiService: APIServiceEntity) -> Bool {
        return apiService.url != nil
    }

    /// 檢查是否至少有一個 API 服務準備好使用
    func apiServiceIsPresent() -> Bool {
        let allServices = getAllAPIServices()
        let readyServices = allServices.filter { isAPIServiceReady($0) }
        return readyServices.count > 0
    }
}
